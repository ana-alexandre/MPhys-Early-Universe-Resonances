...
k = 17  #momentum of fourier mode
mX = 100  #mass of fermion
gphi0 = 1000   #constant
m = 1 #frequency of inflaton oscillations
tmax = np.pi/m * (1/2 + 7)   #large time, equivalent to the +/- infinity limit
tosc_st = np.pi/m * (1/2 + 0)  
tosc = np.pi/m * (1/2 + 6)  #om2 oscillates between -tosc and +tosc (otherwise it is constant)
#h_1 = 
h_max = 1/(1000*max([k,mX,m]))  #step size adjusted to the largest frequency in the system
...
def a(t):
    if (t >= tosc and t <= tmax) or t <= tosc_st:
        ...
        ###set initial values for phi0 and phi'0
        phi0_phi0 = 15 / 28 * m
        return (1 - 2/3 * phi0_phi0 * (t - tosc_st))**(2.0/3)
    else:
...
def dt_a(t):
    if (t >= tosc and t <= tmax) or t <= tosc_st:
        ...
        ###set initial values for phi0 and phi'0
        phi0_phi0 = 15 / 28 * m
        return -4/9 * phi0_phi0 *(1 - 2/3 * phi0_phi0 * (t - tosc_st))**(-1.0/3)
    else:
...
sol_p = solve_ivp(d2up, [0,tmax], np.array([up0,dup0]), t_eval=t_eval, method='RK45',max_step=h_max,atol=atol,rtol=rtol)
sol_m = solve_ivp(d2um, [0,tmax], np.array([um0,dum0]), t_eval=t_eval, method='RK45',max_step=h_max,atol = atol, rtol=rtol)
###################################
...
n_f_std = np.std(n_f)
est_amp_n = 3*n_f_std/np.sqrt(2)
print (r'Final $|\beta|^{2}$: ', n_f_mean, ' +/- ', n_f_std)
print (r'Estimate of amplitude of $|\beta|^{2}$ oscillations: ',est_amp_n)
